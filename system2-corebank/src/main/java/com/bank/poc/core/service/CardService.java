...
public class CardService {
    ...
    public String processTransaction(String cardNumber, String pin, double amount, String type) {
        ...
        if (!hashPin(pin).equals(card.getPinHash())) {
            saveTransaction(cardNumber, card.getCustomerName(), type, amount, "FAILED", "Invalid PIN");
            return "FAILED: Invalid PIN";
        }

        if ("withdraw".equalsIgnoreCase(type)) {
            if (card.getBalance() < amount) {
                saveTransaction(cardNumber, card.getCustomerName(), type, amount, "FAILED", "Insufficient balance");
            ...
            saveTransaction(cardNumber, card.getCustomerName(), type, amount, "FAILED", "Invalid transaction type");
            return "FAILED: Invalid transaction type";
        }

        cardRepository.save(card);
        saveTransaction(cardNumber, card.getCustomerName(), type, amount, "SUCCESS", "Transaction successful");
        return "SUCCESS";
    }

    private void saveTransaction(String cardNumber, String customerName, String type, double amount,
